...
%% 2D plot
scale   =   10;
fig     =   [fig, figure]; set(gcf, 'Position',  [200, 50, 1500, 900]);
rxPos   =   cell2mat({rx.pos}.');
% rxVel   =   cell2mat({rx.vel}.').*scale;
plot(rxPos(:, 1), rxPos(:, 2), 'kv', 'MarkerSize', mrkrSize, 'Color', rxColor, 'LineWidth', mrkrLWidth); 
hold on;
...
for r = radIndFor2D
    for a = 1:azim.steps
        %- Actual positions
        plot(tx(r, a).pos(1), tx(r, a).pos(2), 'kx', 'MarkerSize', mrkrSize, 'Color', txColor, 'LineWidth', mrkrLWidth); 
        hold on;

        %- Estimated positions
        %-- TDoA/FDoA
        plot(estA(r, a).pos(1), estA(r, a).pos(2), 'ko', 'MarkerSize', mrkrSize, 'Color', tdoaColor, 'LineWidth', mrkrLWidth); 
        hold on;
        C       =   cov(txEstPosA(:, :, r, a));
        error_ellipse(C, estA(r, a).pos, 'conf', 0.8,  'style', '--', 'Color', tdoaColor); 

        %-- RSS/DoA
        plot(estB(r, a).pos(1), estB(r, a).pos(2), 'ks', 'MarkerSize', mrkrSize, 'Color', doaColor, 'LineWidth', mrkrLWidth); 
        hold on;
        C       =   cov(txEstPosB(:, :, r, a));
...
L.FontSize = fontSize;
NewHeight = L.Position(4) * lgndHeightScaleFactor;
L.Position(2) = L.Position(2) - (NewHeight - L.Position(4));
L.Position(4) = NewHeight;
